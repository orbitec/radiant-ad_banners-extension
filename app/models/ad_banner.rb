class AdBanner < ActiveRecord::Base
  ...
  def self.select_banner(options = {})
    ...
    sql = "SELECT ad_banners.id,weight FROM ad_banners INNER JOIN assets ON assets.id = ad_banners.asset_id WHERE weight > 0"
    unless matches.blank?
      sql += " AND name like '%#{matches}%'"
    end
    weightings = find_by_sql(sql).map do |banner|
